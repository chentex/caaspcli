...
func createUser(email string, password string) error {
	createUserCmd := exec.Command("/bin/sh", "-c", "'/usr/bin/docker exec -it $(/usr/bin/docker ps -q -f 'name=k8s_velum-dashboard') entrypoint.sh bash -c \"rake velum:create_user['"+email+"','"+password+"']\"'")
